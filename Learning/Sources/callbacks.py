...
def log_rocauc(true, pred, true_val, pred_val, epoch, writer):
    writer.add_scalars('ROC_AUC', {'Train': roc_auc_score(true, pred, multi_class='ovr', average='weighted'), 
                                   'Val': roc_auc_score(true_val, pred_val, multi_class='ovr', average='weighted')}, epoch)
    
def log_precision(true, pred, true_val, pred_val, epoch, writer):
    train_prec = precision_score(true, pred, average=None)
    val_prec = precision_score(true_val, pred_val, average=None)
    writer.add_scalars('Precision', {'Train_0': train_prec[0], 'Train_1': train_prec[1],
                                     'Train_2': train_prec[2], 'Val_0': val_prec[0], 
                                     'Val_1': val_prec[1], 'Val_2': val_prec[2]}, epoch)
    
def log_recall(true, pred, true_val, pred_val, epoch, writer):
    train_rec = recall_score(true, pred, average=None)
    val_rec = recall_score(true_val, pred_val, average=None)
    writer.add_scalars('Recall', {'Train_0': train_rec[0], 'Train_1': train_rec[1],
                                  'Train_2': train_rec[2], 'Val_0': val_rec[0],
                                  'Val_1': val_rec[1], 'Val_2': val_rec[2]}, epoch)
...
class callback(Callback):
    ...
    def on_epoch_end(self, t, p, tv, pv, class_names, running_loss, val_loss, writer):
        ...
        log_rocauc(true, probs, true_val, probs_val, self.epoch, writer)
